...
/// Carrying the timings out to fleetwatch, when this command is the one asked
/// to.
///
...
/// of forwarding them — see `commands::due_to_report` — so nothing happens on a
/// day nobody uses the tracker, and fleetwatch's own staleness is what says so.
///
...
mod fleetwatch {
    ...
    /// A ULID-shaped id, which fleetwatch dedupes on.
    ///
    ...
    /// tracker was quiet but alive.
    fn minted() -> String {
        use std::time::{SystemTime, UNIX_EPOCH};
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default();
        format!("{:016X}{:016X}", now.as_millis() as u64, rand_bits())
    }

    /// Enough entropy to separate two reports minted in the same millisecond.
    fn rand_bits() -> u64 {
    ...
    pub(crate) fn checks(report: &Value) -> Vec<Value> {
        let mut out = Vec::new();
    ...
    pub(crate) async fn send(http: &reqwest::Client, report: &Value) -> Result<()> {
